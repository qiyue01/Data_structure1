...
int main()
{
	...
	cin >> s;
	for (int i = 0; i < 26; ++i)
		part[i].clear_tree(p+10);
	for (int i = 0; i < s.size(); ++i)
		part[s[i] - 'a'].pool[i + 1] = 1;
	for (int i = 0; i < 26; ++i)
		part[i].build(1, 1, s.size());
	int q1, q2, q3;
	int clac[26];
	while (q--)
	{
		...
		for (int i = 0; i < 26; ++i)
		{
			clac[i] = part[i].range_ask_sum_modify(1, q1, q2);
			part[i].range_modify(1, q1, q2, 0);
		}
		if (q3 == 0)
		{
			int i = q1;
			...
			{
				if(clac[j]!=0)
				part[j].range_modify(1, i, i + clac[j] - 1, 1);
				i = i + clac[j];
			}
		}
		else
	...
	for (int i = 0; i < 26; ++i)
		for (int j = 1; j <= p; ++j)
			if (part[i].range_ask_sum_modify(1, j, j) == 1)
				ans[j] = 'a' + i;
	for (int i = 1; i <= p; ++i)
		cout << char(ans[i]);
	return 0;
